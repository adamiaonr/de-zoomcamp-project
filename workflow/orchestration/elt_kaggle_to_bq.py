...
def upload_to_bq(file_path:Path, chunksize:int = 100000) -> None:
  ...
  for _, chunk in enumerate(pd.read_csv(file_path, index_col=False, on_bad_lines='skip', compression='infer', chunksize=chunksize)):
    chunk.to_gbq(destination_table=bq_table_name, project_id=gcp_project_id, credentials=gcp_credentials_block.get_credentials_from_service_account(), chunksize=chunksize, if_exists='append')
